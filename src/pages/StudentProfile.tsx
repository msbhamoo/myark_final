...
const StudentProfile = () => {
  ...
  const { student, isAuthenticated, updateStudent, addXPWithPersist } = useStudentAuth();
  ...
  useEffect(() => {
    const loadBadges = async () => {
      try {
        const badges = await badgesService.getAll();
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (isFirstTime) {
        ...
        toast({ title: "📸 Photo updated!", description: "+25 XP ✨", className: "bg-primary text-primary-foreground border-none" });
      } else {
        toast({ title: "📸 Photo updated!", className: "bg-primary text-primary-foreground border-none" });
      }
    } catch (error) {
  ...
  const handleSaveName = async () => {
    ...
    try {
      ...
      setActiveModal(null);
      toast({ title: "✨ Name updated!", className: "bg-primary text-primary-foreground border-none" });
    } catch (error) {
  ...
  // Show loading if student not loaded yet
  if (!student) {
    return (
    ...
    <div className="min-h-screen bg-background">
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card p-6 rounded-xl">
                  ...
                  {loadingSaved ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <Button className="mt-4" onClick={() => navigate("/")}>Explore Now</Button>
                    </div>
                  )}
                </motion.div>
              )}
      ...
      <AnimatePresence>
        ...
        {activeModal === "interests" && (
          <ModalOverlay onClose={() => setActiveModal(null)}>
            <h3 className="text-xl font-black mb-2">What excites you? 🎯</h3>
