...
export function CategoriesManager() {
  const [items, setItems] = useState<OpportunityCategory[]>([]);
  ...
  useEffect(() => {
    ...
    const savedCategories = localStorage.getItem('opportunity-categories');
    if (savedCategories) {
      setItems(JSON.parse(savedCategories));
    }
    loadItems();
  }, []);
  ...
  const handleEdit = (item: OpportunityCategory) => {
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-border/60 dark:border-white/10 bg-card/80 dark:bg-white/5 p-6 backdrop-blur">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-foreground dark:text-white">Existing Categories</h2>
