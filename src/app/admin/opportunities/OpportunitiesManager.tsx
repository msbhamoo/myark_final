...
export function OpportunitiesManager() {
  ...
  const normalizeOrganizer = (entry: Record<string, unknown>): Organizer => {
    ...
    return {
      ...
      schoolLogoUrl: typeof entry.schoolLogoUrl === 'string' ? entry.schoolLogoUrl : '',
    };
  };
  ...
  useEffect(() => {
    const loadMasters = async () => {
      try {
        const [catRes, orgRes, schoolRes] = await Promise.all([
        ...
        if (orgRes.ok) {
          ...
          allOrganizers.push(...records);
        }

        if (schoolRes.ok) {
          const schoolPayload = await schoolRes.json();
          const schools = (schoolPayload.items ?? []).map((item: any) => ({
            id: item.id,
            name: item.name,
            address: item.address ?? '',
            website: item.website ?? '',
            foundationYear: item.foundationYear ? Number(item.foundationYear) : null,
            type: 'other',
            visibility: 'public',
          ...
          allOrganizers.push(...schools);
        }

        setOrganizers((prev) => {
          const merged = new Map(prev.map((entry) => [entry.id, entry]));
          allOrganizers.forEach((record) => {
            if (!record.id) {
              return;
            }
  ...
  const handleOrganizerChange = async (organizerId: string) => {
    ...
    // Find organizer details from local state and populate fields
