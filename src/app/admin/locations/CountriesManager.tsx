...
export function CountriesManager() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const endpoint = editingId
        ? API_ENDPOINTS.admin.countryById(editingId)
        : API_ENDPOINTS.admin.countries;
  ...
  const handleDelete = async (id: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(API_ENDPOINTS.admin.countryById(id), { method: 'DELETE' });
      if (!response.ok) {
        const body = await response.json().catch(() => ({}));
      ...
      <section className='rounded-2xl border border-border/60 dark:border-white/10 bg-card/80 dark:bg-white/5 p-6 backdrop-blur'>
        ...
        <form onSubmit={handleSubmit} className='mt-6 grid grid-cols-1 gap-4'>
          <div className='space-y-2'>
            <label className='text-sm font-medium text-foreground dark:text-white' htmlFor='country-name'>
              Name *
            </label>
            <Input
              id='country-name'
