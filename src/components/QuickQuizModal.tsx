...
const QUIZ_STEPS = [
    ...
    },
    {
        ...
        options: [
            ...
            { value: "Writing", label: "Storytelling", emoji: "✍️" },
        ],
...
const QuickQuizModal = ({ open, onOpenChange }: QuickQuizModalProps) => {
    const navigate = useNavigate();
    const { student, updateStudent, addXPWithPersist } = useStudentAuth();
    ...
    const handleFinish = async () => {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                navigate("/?filter=for-you&discovery=complete");
            }, 3000);
        } catch (error) {
                                ...
                                {step.options.map((option: any) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            `}>
                                                {Icon ? <Icon className="w-6 h-6" /> : <span>{option.emoji || (typeof option.value === 'number' ? (option.value > 12 ? "🎓" : option.value) : "✨")}</span>}
                                            </div>
