...
import { checkDuplicateOrganiser } from '@/app/admin/organisers/actions';

export function DuplicateOrganiserCheck({ name, excludeId }: { name: string, excludeId?: string }) {
  const [loading, setLoading] = useState(false);
  const [duplicateData, setDuplicateData] = useState<{exists: boolean, matches: { id: string, name: string }[]}>({ exists: false, matches: [] });
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      ...
      setLoading(true);
      try {
        const result = await checkDuplicateOrganiser(name, excludeId);
