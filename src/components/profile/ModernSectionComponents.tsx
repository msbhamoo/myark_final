...
export function ModernAcademicCard({
  ...
  return (
    <div className="group relative rounded-2xl border border-slate-200/60 bg-gradient-to-br from-white/80 via-white/60 to-blue-50/30 p-6 backdrop-blur-xl shadow-lg shadow-blue-200/20 hover:shadow-xl hover:shadow-blue-300/30 transition-all duration-300 dark:from-white/5 dark:via-white/3 dark:to-blue-950/20 dark:border-white/10 dark:shadow-blue-950/40">
      ...
      <div className="relative z-10 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div className="space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              <Badge className="bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-700 font-semibold dark:from-blue-900/40 dark:to-indigo-900/40 dark:text-blue-300">
                {year.grade || 'Academic Record'}
              </Badge>
              {year.session && (
                <Badge variant="outline" className="border-slate-200 text-slate-600 text-xs dark:border-white/20 dark:text-slate-300">
                  {year.session}
                </Badge>
              )}
              {year.board && (
        ...
        {year.summary && (
          <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{year.summary}</p>
        )}
        ...
        {year.teacherComments && (
          <div className="rounded-lg bg-amber-50/60 dark:bg-amber-950/20 border border-amber-200/40 dark:border-amber-900/30 p-3">
            ...
            <p className="text-sm text-amber-900 dark:text-amber-200">{year.teacherComments}</p>
          </div>
        )}
          ...
          <div className="space-y-2">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              {year.subjects.slice(0, 4).map((subject) => (
                <div
                  key={subject.id}
                  className="rounded-lg bg-white/80 dark:bg-white/5 border border-slate-100 dark:border-white/10 p-3 hover:border-indigo-200 dark:hover:border-indigo-900/40 transition-colors"
                >
                  <div className="flex items-center justify-between gap-2 mb-2">
                    ...
                    {subject.grade && (
                      <Badge className="bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300 text-xs font-bold">
                        {subject.grade}
                      </Badge>
                    )}
                  </div>
...
export function ModernAchievementCard({
  ...
  return (
    <div className="group rounded-2xl border border-amber-200/40 bg-gradient-to-br from-amber-50/80 via-white/60 to-orange-50/40 p-6 backdrop-blur-xl shadow-lg shadow-amber-200/20 hover:shadow-xl hover:shadow-amber-300/30 transition-all duration-300 dark:from-amber-950/20 dark:via-white/3 dark:to-orange-950/20 dark:border-amber-900/30 dark:shadow-amber-950/40">
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div className="space-y-3 flex-1">
            <div className="flex flex-wrap items-center gap-2">
              <Badge className="flex items-center gap-1.5 bg-gradient-to-r from-amber-100 to-orange-100 text-amber-900 dark:from-amber-900/40 dark:to-orange-900/40 dark:text-amber-300">
                <Trophy className="h-3.5 w-3.5" />
                {achievement.title}
              </Badge>
              {achievement.level && (
                <Badge variant="outline" className="border-slate-200 bg-slate-50 text-xs font-medium text-slate-600 dark:border-white/10 dark:bg-white/5 dark:text-slate-300">
              ...
              {achievement.approved && (
                <Badge className="flex items-center gap-1 border border-emerald-200 bg-emerald-50 text-xs font-medium text-emerald-600 dark:border-emerald-900/40 dark:bg-emerald-950/20 dark:text-emerald-400">
            ...
            {achievement.description && (
              <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{achievement.description}</p>
            )}
...
export function ModernCompetitionCard({
  ...
  return (
    <div className="group rounded-2xl border border-purple-200/40 bg-gradient-to-br from-purple-50/80 via-white/60 to-pink-50/40 p-6 backdrop-blur-xl shadow-lg shadow-purple-200/20 hover:shadow-xl hover:shadow-purple-300/30 transition-all duration-300 dark:from-purple-950/20 dark:via-white/3 dark:to-pink-950/20 dark:border-purple-900/30 dark:shadow-purple-950/40">
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div className="space-y-3 flex-1">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">{competition.name}</h3>
            {competition.description && (
              <p className="text-sm text-slate-700 dark:text-slate-300">{competition.description}</p>
            )}
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              {competition.status && (
                <Badge className={cn('text-xs font-semibold', statusColors[competition.status] || 'bg-slate-100 text-slate-700')}>
...
export function ModernExtracurricularCard({
  ...
  return (
    <div className="group rounded-2xl border border-emerald-200/40 bg-gradient-to-br from-emerald-50/80 via-white/60 to-teal-50/40 p-6 backdrop-blur-xl shadow-lg shadow-emerald-200/20 hover:shadow-xl hover:shadow-emerald-300/30 transition-all duration-300 dark:from-emerald-950/20 dark:via-white/3 dark:to-teal-950/20 dark:border-emerald-900/30 dark:shadow-emerald-950/40">
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div className="space-y-3 flex-1">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              {activity.status && (
                <Badge className={cn('text-xs font-semibold', statusColors[activity.status] || 'bg-slate-100 text-slate-700')}>
...
export function SubjectPerformanceCard({
  ...
  return (
    <div className={cn(
      'rounded-xl bg-gradient-to-br p-4 border border-white/20 backdrop-blur-xs shadow-md',
      getGradeColor(percent)
    )}>
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm font-semibold truncate">{name}</span>
        <Badge className="bg-white/30 backdrop-blur-sm font-bold text-xs">
          {percent ?? 'N/A'}%
        </Badge>
      </div>
      <Progress
